            ...
            for (let i = 0; i < notificationCount; i++) {
                ...
                switch (type) {
                    case 'event_invite':
                        ...
                        // Get sender image
                        try {
                            const sender = yield (0, index_1.getAsync)('SELECT profile_image FROM users WHERE id = ?', [senderId]);
                            if (sender) {
                    ...
                    case 'event_update':
                        ...
                        try {
                            const event = yield (0, index_1.getAsync)('SELECT creator_id FROM events WHERE id = ?', [relatedId]);
                            if (event) {
                ...
                yield (0, index_1.runAsync)(`INSERT INTO notifications (
...
function seedDatabase() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            yield (0, index_1.runAsync)('PRAGMA foreign_keys = OFF');
            yield (0, index_1.runAsync)('DELETE FROM messages');
            yield (0, index_1.runAsync)('DELETE FROM conversation_participants');
            yield (0, index_1.runAsync)('DELETE FROM conversations');
            yield (0, index_1.runAsync)('DELETE FROM notifications');
            yield (0, index_1.runAsync)('DELETE FROM comment_votes');
            yield (0, index_1.runAsync)('DELETE FROM discussion_votes');
            yield (0, index_1.runAsync)('DELETE FROM comments');
            yield (0, index_1.runAsync)('DELETE FROM bookmarks');
            yield (0, index_1.runAsync)('DELETE FROM event_participants');
            yield (0, index_1.runAsync)('DELETE FROM discussions');
            yield (0, index_1.runAsync)('DELETE FROM events');
            yield (0, index_1.runAsync)('DELETE FROM users');
            // Re-enable foreign keys
            yield (0, index_1.runAsync)('PRAGMA foreign_keys = ON');
            console.log('✅ Cleared existing data');
            // Hash passwords
            const password1 = yield hashPassword('password123');
            const password2 = yield hashPassword('password456');
            const password3 = yield hashPassword('password789');
            const password4 = yield hashPassword('password321');
            const password5 = yield hashPassword('password654');
            const password6 = yield hashPassword('password987');
            const password7 = yield hashPassword('password135');
            const password8 = yield hashPassword('password246');
            const password9 = yield hashPassword('password579');
            const password10 = yield hashPassword('password024');
            const password11 = yield hashPassword('password468');
            const password12 = yield hashPassword('password802');
            const password13 = yield hashPassword('password913');
            const password14 = yield hashPassword('password357');
            const password15 = yield hashPassword('password159');
            // Insert users
            const users = [
                {
                    username: 'john_doe',
            ...
            for (const user of users) {
                yield (0, index_1.runAsync)(`INSERT INTO users (username, email, password, full_name, bio, profile_image) 
         VALUES (?, ?, ?, ?, ?, ?)`, [user.username, user.email, user.password, user.full_name, user.bio, user.profile_image]);
            }
            ...
            for (const event of events) {
                yield (0, index_1.runAsync)(`INSERT INTO events (creator_id, title, description, sport_type, location, start_date, end_date, max_players, current_players, image_url) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
                    event.creator_id,
                    event.title,
                    event.description,
            ...
            for (const participant of participants) {
                yield (0, index_1.runAsync)(`INSERT INTO event_participants (event_id, user_id, status)
         VALUES (?, ?, ?)`, [participant.event_id, participant.user_id, participant.status]);
            }
            ...
            for (const comment of comments) {
                yield (0, index_1.runAsync)(`INSERT INTO comments (event_id, user_id, content, parent_comment_id, thumbs_up, thumbs_down) 
         VALUES (?, ?, ?, ?, ?, ?)`, [comment.event_id, comment.user_id, comment.content, comment.parent_comment_id || null, comment.thumbs_up || 0, comment.thumbs_down || 0]);
            }
            ...
            for (const vote of commentVotes) {
                try {
                    yield (0, index_1.runAsync)(`INSERT INTO comment_votes (comment_id, user_id, vote_type) 
           VALUES (?, ?, ?)`, [vote.comment_id, vote.user_id, vote.vote_type]);
                }
                catch (error) {
            ...
            for (const bookmark of bookmarks) {
                yield (0, index_1.runAsync)(`INSERT INTO bookmarks (event_id, user_id) 
         VALUES (?, ?)`, [bookmark.event_id, bookmark.user_id]);
            }
            ...
            for (const notification of notifications) {
                yield (0, index_1.runAsync)(`INSERT INTO notifications (user_id, type, content, related_id, is_read, created_at) 
         VALUES (?, ?, ?, ?, ?, ?)`, [notification.user_id, notification.type, notification.content, notification.related_id, notification.is_read, notification.created_at]);
            }
            ...
            for (const conversation of conversations) {
                yield (0, index_1.runAsync)(`INSERT INTO conversations (id, created_at)
         VALUES (?, ?)`, [conversation.id, conversation.created_at]);
            ...
            for (const participant of conversationParticipants) {
                yield (0, index_1.runAsync)(`INSERT INTO conversation_participants (conversation_id, user_id, joined_at)
         VALUES (?, ?, ?)`, [participant.conversation_id, participant.user_id, participant.joined_at]);
            }
            ...
            for (const message of messages) {
                yield (0, index_1.runAsync)(`INSERT INTO messages (id, conversation_id, sender_id, content, created_at) 
         VALUES (?, ?, ?, ?, ?)`, [message.id, message.conversation_id, message.sender_id, message.content, message.created_at]);
            }
            console.log('✅ Sample messages created');
            // Insert dashboard related data
            try {
                // Create dashboard_courses table
                yield (0, index_1.runAsync)(`
      ...
      `);
                ...
                for (const course of dashboardCourses) {
                    yield (0, index_1.runAsync)(`INSERT INTO dashboard_courses (
            title, instructor, instructor_avatar, level, rating, attendees, image_url
                ...
                for (const course of userCourses) {
                    const result = yield (0, index_1.runAsync)(`INSERT INTO user_courses (
            user_id, title, sessions_completed, total_sessions, image_url
                    ...
                    for (const participant of participants) {
                        yield (0, index_1.runAsync)(`INSERT INTO course_participants (course_id, user_id) VALUES (?, ?)`, [result.lastID, participant]);
                    }
                }
                console.log('✅ User courses and participants created');
                // Seed user_statistics for user 1
                yield (0, index_1.runAsync)(`INSERT INTO user_statistics (
          ...
          total_students, total_students_change, total_hours, total_hours_change
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [1, 210, '+15%', 34, '+15%', 17, '-2%', 11, '-9%']);
                ...
                for (let i = 0; i < days.length; i++) {
                    yield (0, index_1.runAsync)(`INSERT INTO productivity_data (
            user_id, day, mentoring, self_improve, student
          ) VALUES (?, ?, ?, ?, ?)`, [1, days[i], mentoring[i], selfImprove[i], student[i]]);
                }
                ...
                for (const event of upcomingEvents) {
                    yield (0, index_1.runAsync)(`INSERT INTO upcoming_events (
            user_id, title, type, event_date, event_time
                ...
                for (const discussion of discussions) {
                    yield (0, index_1.runAsync)(`INSERT INTO discussions (title, content, category, creator_id, votes_up, votes_down, image_url)
          VALUES (?, ?, ?, ?, ?, ?, ?)`, [discussion.title, discussion.content, discussion.category, discussion.creator_id,
                ...
                for (const vote of discussionVotes) {
                    yield (0, index_1.runAsync)(`INSERT INTO discussion_votes (discussion_id, user_id, vote_type)
          VALUES (?, ?, ?)`, [vote.discussion_id, vote.user_id, vote.vote_type]);
                }
                ...
                for (const comment of discussionComments) {
                    yield (0, index_1.runAsync)(`INSERT INTO comments (discussion_id, user_id, content, parent_comment_id, thumbs_up, thumbs_down)
          VALUES (?, ?, ?, ?, ?, ?)`, [comment.discussion_id, comment.user_id, comment.content,
